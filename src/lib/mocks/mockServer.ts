...
/**
 * Register a team — validates payload and returns a fake team ID.
 */
export async function mockRegister(
  payload: RegistrationPayload,
): Promise<RegistrationResult> {
  await simulateDelay(600); // Simulate longer server processing

  // Basic server-side validation
  if (!payload.teamName || !payload.leaderEmail || !payload.trackId) {
    return { success: false, teamId: '', message: 'Missing required fields.' };
  }

  const fakeTeamId = `TM-${Math.floor(1000 + Math.random() * 9000)}`;
  return {
    success: true,
    teamId: fakeTeamId,
    message: `Team "${payload.teamName}" registered successfully!`,
  };
}
